...
if($resultado == 1 && $arquivo['size'] <= 5242880){



if($resultado1 == 0 && $resultado2 == 0 && $resultado3 == 0 && $resultado4 == 0){
$sql= "INSERT INTO `usuarios`(`nome`, `cpf`,`rg`,`sexo`,`email`,`tag`,`telefone`,`nivel`) VALUES ('$nome', '$cpf', '$rg','$sexo', '$email','$tag', '$telefone','2')";
...
$sql1 = "SELECT `id` FROM `usuarios` WHERE `tag`='$tag'";
...
<center>
<a href="adicionar_produto2.php" role="button" class="btn btn-sm btn-primary">Cadastrar um novo usuario</a>
</center>
</div>
...
<?php } ?>
	...
	<a href="adicionar_produto2.php?nome=<?php echo $nome?>&cpf=<?php echo $cpf ?>&rg=<?php echo $rg ?>&email=<?php echo $email ?>&tag=<?php echo $tag ?>&telefone=<?php echo $telefone ?>&sexo=<?php echo $sexo ?>"" role="button" class="btn btn-sm  btn-warning" style="color: #fff">Voltar</a>
...
<div style="padding-top: 20px">	
<center> 
<a href="adicionar_produto2.php?nome=<?php echo $nome?>&cpf=<?php echo $cpf ?>&rg=<?php echo $rg ?>&email=<?php echo $email ?>&tag=<?php echo $tag ?>&telefone=<?php echo $telefone ?>&sexo=<?php echo $sexo ?>"" role="button" class="btn btn-sm btn-primary">Voltar</a>
</center>
</div>
...
function analisar_arquivo($arquivo){
	
include 'conexao.php';

$arquivo_principal = $arquivo['name'];
$extensao = strtolower(pathinfo($arquivo_principal,PATHINFO_EXTENSION));
